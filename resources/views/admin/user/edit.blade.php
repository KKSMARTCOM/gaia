            ...
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <form
                                action="{{ $user && $user->id ? route('admin.users.update', $user->id) : route('admin.users.store') }}"
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" class="form-control" required>
                                        <option value="1"
                                            {{ old('status', $user->status ?? 1) == 1 ? 'selected' : '' }}>Actif</option>
                                        <option value="0"
                                            {{ old('status', $user->status ?? 1) == 0 ? 'selected' : '' }}>Inactif</option>
                                    </select>
                                </div>

                                <button type="submit"
                                    class="btn btn-success">{{ $user && $user->id ? 'Enregistrer' : 'Ajouter' }}</button>
                            </form>
                            ...
                            @if ($user && $user->id)
                                ...
                                <form action="{{ route('admin.users.destroy', $user->id) }}" method="POST"
                                    ...
                                    <button type="submit" class="btn btn-danger"
                                        onclick="return confirm('Êtes-vous sûr de vouloir supprimer cet utilisateur ?')">Supprimer
                                        cet utilisateur</button>
                                </form>
                            @endif
                        </div>
                    </div>
